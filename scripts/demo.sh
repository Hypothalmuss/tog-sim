...
setsid ros2 launch togsim_bringup sim_full.launch.py gui:=true infeed_speed:=$speed outfeed_speed:=$speed product_rate:=${DEMO_RATE:-30.0} product_classes:=${DEMO_CLASSES:-product_bar,product_carton} motion_profile:=${DEMO_PROFILE:-smooth} >"$log/sim.log" 2>&1 </dev/null &
