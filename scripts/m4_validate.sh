...
if [ "$mode" = vision ]; then
  ...
  setsid ros2 launch togsim_perception perception.launch.py ${M4_WEIGHTS:+weights:=$M4_WEIGHTS} ${M4_TRAYS:+tray_models:=$M4_TRAYS} >"$out/perception.log" 2>&1 </dev/null &
...
timeout ${M4_TIMEOUT:-900} ros2 run togsim_task run_cycle --ros-args --params-file "$(ros2 pkg prefix togsim_task)/share/togsim_task/config/task_tuning.yaml" -p perception:=$mode -p continuous:=true -p cycles:="$cycles" -p belt_speed:=$speed -p outfeed_speed:=${M4_OUTFEED:-0.0} -p use_sim_time:=true >"$out/run_cycle.log" 2>&1
