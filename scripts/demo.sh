...
launch_cell() {  # Fortress occasionally hangs at robot spawn (no joint states): relaunch once
  for attempt in 1 2; do
    ...
    setsid ros2 launch togsim_bringup sim_full.launch.py gui:=true infeed_speed:=$speed outfeed_speed:=${DEMO_OUTFEED:-$speed} product_rate:=${DEMO_RATE:-30.0} product_classes:=${DEMO_CLASSES:-product_bar,product_carton} ${DEMO_TRAYS:+tray_models:=$DEMO_TRAYS} ${DEMO_PITCH:+tray_pitch:=$DEMO_PITCH} motion_profile:=${DEMO_PROFILE:-smooth} >"$log/sim.log" 2>&1 </dev/null &
...
setsid ros2 run togsim_perception conveyor_tracker --ros-args --params-file "$(ros2 pkg prefix togsim_perception)/share/togsim_perception/config/perception_tuning.yaml" -p use_sim_time:=true -p source:=$mode >"$log/tracker.log" 2>&1 </dev/null &
